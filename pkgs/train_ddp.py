...
class trainer():
    ...
    def __init__(self, device, data_in, labels, filename='model.pt', kn=0.5, lr=10**-3, ngrid=30) -> None:
    ...
    def train(self, kE = 0.9, steps=10, batch_size = 50) -> float:
        ...
        for _ in range(steps):  # outer loop of training steps
            ...
            minibatch, labels = self.sampler.sample(batch_size=batch_size, i_molecule=self.device);
