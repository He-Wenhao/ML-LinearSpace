...
class trainer():
    ...
    def __init__(self, device, data_in, labels, op_matrices=[], 
                 filename='model.pt', lr=10**-3, scaling = 0.1,
                 element_list = ['C','H']) -> None:
        ...
        self.model = V_theta(device, self.irreps).to(device)
        self.optim = torch.optim.Adam(self.model.parameters(), lr=lr)
        self.filename = filename;
        self.sampler = sampler(data_in, labels, device);
        self.n_molecules = len(data_in);
        self.transformer = to_mat(device, self.irreps);
...
class trainer_ddp():
    ...
    def __init__(self, device, data_in, labels, 
                 ...
                 scaling=0.1, element_list = ['C','H']) -> None:
        ...
        self.model = DDP(V_theta(device,self.irreps).to(device), device_ids=[device]);
        self.optim = torch.optim.Adam(self.model.parameters(), lr=lr)
        ...
        for i in range(self.n_molecules):
            elements = data_in[i]['elements'];
            orbitals_list = [nbasis[ele] for ele in elements];
            map1 = [sum(orbitals_list[:j]) for j in range(len(elements)+1)];
            mati = [];
            for j in range(len(elements)):
                Sm = torch.zeros_like(labels[i]['Smhalf']);
